import React from 'react';
import { CheckCircle, Zap, Award, Clock, Target, Trophy } from 'lucide-react';

interface ProfileStatsProps {
  stats: {
    challengesSolved: number;
    totalChallenges: number;
    totalXP: number;
    certificationsUnlocked: number;
    timeSpent: string;
    streakDays: number;
    averageRating: number;
  };
}

interface StatCardProps {
  icon: React.ReactNode;
  label: string;
  value: string;
  subValue?: string;
  color: string;
}

const StatCard: React.FC<StatCardProps> = ({ icon, label, value, subValue, color }) => {
  return (
    <div className="bg-gray-900 rounded-xl p-6 border border-gray-700 shadow-2xl hover:border-gray-600 transition-all duration-300 group">
      <div className="flex items-center gap-4">
        <div className={`w-12 h-12 rounded-lg flex items-center justify-center shadow-lg ${color} group-hover:scale-110 transition-transform duration-300`}>
          {icon}
        </div>
        <div className="flex-1">
          <p className="text-2xl font-bold text-white mb-1">{value}</p>
          <p className="text-gray-400 text-sm font-medium">{label}</p>
          {subValue && (
            <p className="text-green-400 text-xs font-semibold mt-1">{subValue}</p>
          )}
        </div>
      </div>
    </div>
  );
};

export const ProfileStats: React.FC<ProfileStatsProps> = ({ stats }) => {
  const statCards = [
    {
      icon: <CheckCircle className="w-6 h-6 text-white" />,
      label: "Challenges Solved",
      value: stats.challengesSolved.toString(),
      subValue: `${Math.round((stats.challengesSolved / stats.totalChallenges) * 100)}% completion`,
      color: "bg-gradient-to-br from-green-500 to-green-600"
    },
    {
      icon: <Zap className="w-6 h-6 text-white" />,
      label: "Total XP",
      value: stats.totalXP.toLocaleString(),
      color: "bg-gradient-to-br from-blue-500 to-purple-500"
    },
    {
      icon: <Award className="w-6 h-6 text-white" />,
      label: "Certifications",
      value: stats.certificationsUnlocked.toString(),
      color: "bg-gradient-to-br from-yellow-500 to-orange-500"
    },
    {
      icon: <Clock className="w-6 h-6 text-white" />,
      label: "Time Spent",
      value: stats.timeSpent,
      color: "bg-gradient-to-br from-purple-500 to-pink-500"
    },
    {
      icon: <Target className="w-6 h-6 text-white" />,
      label: "Current Streak",
      value: `${stats.streakDays} days`,
      color: "bg-gradient-to-br from-red-500 to-rose-500"
    },
    {
      icon: <Trophy className="w-6 h-6 text-white" />,
      label: "Avg Rating",
      value: `${stats.averageRating.toFixed(1)}★`,
      color: "bg-gradient-to-br from-indigo-500 to-blue-500"
    }
  ];

  return (
    <div>
      <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-3">
        <div className="w-8 h-8 bg-gradient-to-br from-green-400 to-blue-500 rounded-lg flex items-center justify-center">
          <Zap className="w-4 h-4 text-white" />
        </div>
        Performance Stats
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {statCards.map((stat, index) => (
          <StatCard key={index} {...stat} />
        ))}
      </div>
    </div>
  );
};